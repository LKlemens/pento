...
defmodule Pento.Accounts.UserToken do
  ...
  def build_session_token(user) do
    ...
    {token, %Pento.Accounts.UserToken{token: token, context: "session", user_id: user.id}}
  end
  ...
  def token_and_context_query(token, context) do
    from Pento.Accounts.UserToken, where: [token: ^token, context: ^context]
  end
  ...
  def user_and_contexts_query(user, :all) do
    from t in Pento.Accounts.UserToken, where: t.user_id == ^user.id
  end
